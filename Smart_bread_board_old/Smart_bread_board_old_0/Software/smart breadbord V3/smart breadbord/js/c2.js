...
//code edited by vikrant
//------------------------------------------------------------------------------------------
// Transformer  Class
//------------------------------------------------------------------------------------------

function Transformer(node1,nodeCurrent,node2,values){
	//this.volts=volts;
	this.node1=node1;
...
Transformer.prototype.matrix=function(dt,time,system,tprev,told){
    ...
    if(this.type=="triangle"){
    	...
    	if(normTime<.25) t=.5*(this.min+this.max)+2*normTime*(this.max-this.min);
    	else if(normTime<.75) t=this.max-(normTime-.25)*2*(this.max-this.min);
    	//else if(normTime<.75) t=-.5*(this.min+this.max);//-(normTime-.5)*(this.max-this.min);
    	else t=this.min+(normTime-.75)*2*(this.max-this.min);
    	//else if(normTime<.5) t=(.25*this.min+.75*this.max)-(normTime-.25)*(this.max-this.min);
    }else if(this.type=="square"){
    	t=this.min;
    	if( time%this.period>this.period*.5) t=this.max;
    }else if(this.type=="sin"){
    	t=(Math.sin(time*2*Math.PI/this.period)+1)*(this.max-this.min)*.5+this.min
    }
    system.addToB(this.nodeCurrent,t-(tprev[this.node1]-tprev[this.node2]));

    // nodeCurrent is the current through the transformer source. it needs to be added to the KVL of node 1 and node2
    system.addToMatrix(this.node1,this.nodeCurrent,-1);
    system.addToMatrix(this.node2,this.nodeCurrent,1);
    system.addToB(this.node1,tprev[this.nodeCurrent]);
    system.addToB(this.node2,-tprev[this.nodeCurrent]);
}
...
Circuit.prototype.allocNode=function(name){
    if(!(name in this.nameToNode)){
        ...
        if(name=="GND"){
        	this.ground=num;
			alert("conected to ground");
        }
        return num;
		if(name=="V"){
        	this.voltage=num;
			alert("connected to voltsge");
        }
		return num;
		if(name=="T"){
        	this.transformer=num;
			alert("conected to transformer");
        }
		return num;
		if(name=="R"){
        	this.register=num;
			alert("conected to register");
        }
        return num;
		return num;
		if(name=="L"){
        	this.load=num;
			alert("conected to lode");
        }
		return num;
		if(name=="C"){
        	this.capacitor=num;
			alert("conected to capicator");
        }
        return num;
		if(name=="D"){
        	this.diode=num;
			alert("conected to diode");
        }
       
		return num;
		if(name=="NPN"){
        	this.transister=num;
			alert("conected to trans");
        }
        return num;
    }else{
		alert(this.nameToNode[name]);
...
//code edited by vikrant
Circuit.prototype.addTransformer=function(name1,name2,value){
    n1=this.allocNode(name1)
    n12=this.allocNode("sourceCurrent"+this.num)
    n2=this.allocNode(name2)
    this.components.push(new (n1,n12,n2,value))
}
